...
if (base) {
  ...
  const client = sign({v: 1, role: 'client', exp: now + 3600, styleSet: 'graphite', palette: 'navy-brass', view: 'design'})
  ...
  const path = '/site-preview/graphite/navy-brass/design'
  const tree = encodeURIComponent(JSON.stringify(['', {children: ['(preview)', {children: ['site-preview', {children: [['styleSet', 'graphite', 'd'], {children: [['palette', 'navy-brass', 'd'], {children: [['view', 'design', 'd'], {children: ['__PAGE__', {}]}]}]}]}]}]}, null, null, true]))
  ...
  check(enter.res.headers.get('location')?.endsWith('/site-preview/site/site/design'), `a good link went to ${enter.res.headers.get('location')}`)
  ...
  const switched = await get('/site-preview/marble/black-gold/design', {...cookie(operator), RSC: '1'})
  ...
  // The grey box.
  const grey = await get('/site-preview/site/site/grey', cookie(operator))
  ...
  // A client.
  const bound = await get('/site-preview/site/site/design', cookie(client))
  ...
  check(!cl.body.includes('/site-preview/marble/') && !cl.body.includes('/#/design?t='), 'the client view shows the roster or Apply')
  ...
  console.log(`served: ${base} checked as a visitor, without a cookie three ways, with bad and good links, as the operator, the client and an ended session`)
}
