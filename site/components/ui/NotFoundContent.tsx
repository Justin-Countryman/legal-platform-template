 ...
 *
 * There are two because `app/review/[slug]` sits OUTSIDE the `(site)` route
 * group, so `app/(site)/not-found.tsx` does not cover it — verified on a real
 * render, where `/review/<missing>` fell through to Next's default 404 and the
 * bare firm name while every `(site)` URL was already correct.
 */
