...
export const locationPage = defineType({
  ...
  fields: [
    // ─── Page Settings ────────────────────────────────────────────────────────
    {
      ...
      type: 'reference',
      to: [{type: 'location'}],
      description: 'Powers: firm name, address, phone, hours, LocalBusiness schema',
      validation: (Rule) => Rule.required().warning('Location record reference is required'),
    },
    {
      name: 'title',
      fieldset: 'pageSettings',
      title: 'Page Title',
      type: 'string',
      description: 'Short internal title used to generate the slug — e.g. "[City] Law Office"',
    ...
    },
    {
      ...
      title: 'OG Image',
      type: 'image',
      description: '1200×630 px recommended. Optional — when empty, the site generates a branded share image automatically from the firm name, logo and brand color, so leaving this blank is a valid choice rather than a gap.',
      fieldset: 'seo',
    ...
    },
    {
      ...
      title: 'Body Content',
      type: 'blockContent',
      description: 'Optional content above the data section pulled from location record',
    },
    {
      name: 'mapEmbed',
      ...
      type: 'text',
      rows: 4,
      description: 'Google Maps embed — entered manually in Sanity Studio per location',
    },
    // ─── Sidebar ──────────────────────────────────────────────────────────────
    {
      name: 'sidebar',
